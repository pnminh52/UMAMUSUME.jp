...
const Header = () => {
  ...
  {menuItems.map((item) => (
    <li
      key={item.label}
      className="relative" // để dropdown position: absolute dựa vào parent
      onMouseEnter={() => item.label === "Contents" && setOpenDropdown(true)}
      onMouseLeave={() => item.label === "Contents" && setOpenDropdown(false)}
    >
      {/* Link chính */}
      <Link
        to={item.path}
        onClick={() => setShowPopup(false)}
        className="text-lg italic text-white flex items-center gap-1 font-semibold"
      >
        {umaSvg} {item.label}
      </Link>

      {/* Dropdown chỉ cho Contents */}
      {item.label === "Contents" && openDropdown && (
        <ul className="absolute top-full -right-10 bg-[#0F5AFF] p-4 flex flex-col gap-2 min-w-[150px] z-50 whitespace-nowrap shadow-lg">
          {options.map((opt) => (
            <li key={opt.label}>
              <Link
                to={`/contents/${opt.path}`}
                className="text-white italic font-semibold hover:underline block px-2 py-1"
              >
                {opt.label}
              </Link>
            </li>
